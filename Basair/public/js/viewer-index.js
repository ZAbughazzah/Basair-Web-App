...
export async function injectQuranPage(page) {
    ...
    for (let i = page.startingSura; i <= page.endingSura; i++) {
        ...
        if (surahXMehwarObj.length != 0) {
            const surahSections = surahXMehwarObj[0].mahawer.map(m => m.sections).flat(Infinity);
            let ayaatXSections = '';
            ...
            // console.log(surahXMehwarObj[0].moqadema.sections)
            surahTables += `
                <table class="main-table-viewer">
                    ...
                    ${page.startingAya === 1 ?
                surahXMehwarObj[0].moqadema.sections.map(s =>
                    ayaatXSectionHTML(
